...
#ifndef DAPScoin_ZDAPSWALLET_H
#define DAPScoin_ZDAPSWALLET_H
...
public:
    CzDAPSWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZDAPS(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZDAPS(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //DAPScoin_ZDAPSWALLET_H
