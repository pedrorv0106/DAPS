...
CzDAPSTracker::CzDAPSTracker(std::string strWalletFile)
{
...
CzDAPSTracker::~CzDAPSTracker()
{
    mapSerialHashes.clear();
    mapPendingSpends.clear();
}

void CzDAPSTracker::Init()
{
...
bool CzDAPSTracker::Archive(CMintMeta& meta)
{
...
bool CzDAPSTracker::UnArchive(const uint256& hashPubcoin, bool isDeterministic)
{
    CWalletDB walletdb(strWalletFile);
    if (isDeterministic) {
        CDeterministicMint dMint;
...
CMintMeta CzDAPSTracker::Get(const uint256 &hashSerial)
{
...
CMintMeta CzDAPSTracker::GetMetaFromPubcoin(const uint256& hashPubcoin)
{
...
bool CzDAPSTracker::GetMetaFromStakeHash(const uint256& hashStake, CMintMeta& meta) const
{
    for (auto& it : mapSerialHashes) {
...
std::vector<uint256> CzDAPSTracker::GetSerialHashes()
{
...
CAmount CzDAPSTracker::GetBalance(bool fConfirmedOnly, bool fUnconfirmedOnly) const
{
    CAmount nTotal = 0;
...
CAmount CzDAPSTracker::GetUnconfirmedBalance() const
{
    return GetBalance(false, true);
}

std::vector<CMintMeta> CzDAPSTracker::GetMints(bool fConfirmedOnly) const
{
...
bool CzDAPSTracker::HasMintTx(const uint256& txid)
{
    for (auto it : mapSerialHashes) {
...
bool CzDAPSTracker::HasPubcoin(const CBigNum &bnValue) const
{
...
bool CzDAPSTracker::HasPubcoinHash(const uint256& hashPubcoin) const
{
    for (auto it : mapSerialHashes) {
        CMintMeta meta = it.second;
        if (meta.hashPubcoin == hashPubcoin)
...
bool CzDAPSTracker::HasSerial(const CBigNum& bnSerial) const
{
    uint256 hash = GetSerialHash(bnSerial);
    return HasSerialHash(hash);
}

bool CzDAPSTracker::HasSerialHash(const uint256& hashSerial) const
{
    auto it = mapSerialHashes.find(hashSerial);
    return it != mapSerialHashes.end();
}

bool CzDAPSTracker::UpdateZerocoinMint(const CZerocoinMint& mint)
{
...
bool CzDAPSTracker::UpdateState(const CMintMeta& meta)
{
    CWalletDB walletdb(strWalletFile);
...
void CzDAPSTracker::Add(const CDeterministicMint& dMint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzDAPSTracker::Add(const CZerocoinMint& mint, bool isNew, bool isArchived)
{
    CMintMeta meta;
...
void CzDAPSTracker::SetPubcoinUsed(const uint256& hashPubcoin, const uint256& txid)
{
...
void CzDAPSTracker::SetPubcoinNotUsed(const uint256& hashPubcoin)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
void CzDAPSTracker::RemovePending(const uint256& txid)
{
    uint256 hashSerial;
...
bool CzDAPSTracker::UpdateStatusInternal(const std::set<uint256>& setMempool, CMintMeta& mint)
{
...
std::set<CMintMeta> CzDAPSTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrint("zero", "%s: added %d dzdaps from DB\n", __func__, listDeterministicDB.size());
    }
