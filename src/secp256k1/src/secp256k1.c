...
int secp256k1_ecdsa_signature_parse_der(const secp256k1_context* ctx, secp256k1_ecdsa_signature* sig, const unsigned char *input, size_t inputlen) {
    ...
    if (secp256k1_ecdsa_sig_parse2(&r, &s, input, inputlen)) {
...
int secp256k1_ecdsa_signature_serialize_der(const secp256k1_context* ctx, unsigned char *output, size_t *outputlen, const secp256k1_ecdsa_signature* sig) {
    ...
    return secp256k1_ecdsa_sig_serialize2(output, outputlen, &r, &s);
}
...
int secp256k1_ecdsa_verify2(const secp256k1_context* ctx, const secp256k1_ecdsa_signature *sig, const unsigned char *msg32, const secp256k1_pubkey *pubkey) {
    ...
    return (!secp256k1_scalar_is_high(&s) &&
            ...
            secp256k1_ecdsa_sig_verify2(&ctx->ecmult_ctx, &r, &s, &q, &m));
}
    ...
    if (!overflow && !secp256k1_scalar_is_zero(&sec)) {
        ...
        while (1) {
            ...
            if (!overflow && !secp256k1_scalar_is_zero(&non)) {
                if (secp256k1_ecdsa_sig_sign2(&ctx->ecmult_gen_ctx, &r, &s, &sec, &msg, &non, NULL)) {
                    break;
                }
            }
            count++;
...
int secp256k1_ec_pubkey_create2(const secp256k1_context* ctx, secp256k1_pubkey *pubkey, const unsigned char *seckey) {
    ...
    ret = (!overflow) & (!secp256k1_scalar_is_zero(&sec));
    if (ret) {
        secp256k1_ecmult_gen2(&ctx->ecmult_gen_ctx, &pj, &sec);
...
int secp256k1_ec_pubkey_tweak_add2(const secp256k1_context* ctx, secp256k1_pubkey *pubkey, const unsigned char *tweak) {
    ...
    memset(pubkey, 0, sizeof(*pubkey));
    if (ret) {
        if (secp256k1_eckey_pubkey_tweak_add2(&ctx->ecmult_ctx, &p, &term)) {
            secp256k1_pubkey_save(pubkey, &p);
        } else {
            ret = 0;
        }
...
int secp256k1_ec_pubkey_tweak_mul2(const secp256k1_context* ctx, secp256k1_pubkey *pubkey, const unsigned char *tweak) {
    ...
    memset(pubkey, 0, sizeof(*pubkey));
    if (ret) {
        if (secp256k1_eckey_pubkey_tweak_mul2(&ctx->ecmult_ctx, &p, &factor)) {
            secp256k1_pubkey_save(pubkey, &p);
        } else {
            ret = 0;
        }
