...
void test_exhaustive_addition(const secp256k1_ge *group, const secp256k1_gej *groupj, int order) {
    ...
    for (j = 0; j < order; j++) {
        ...
        for (i = 0; i < order; i++) {
            ...
            secp256k1_gej tmp;
            /* add_var */
            secp256k1_gej_add_var2(&tmp, &groupj[i], &groupj[j], NULL);
            ...
            /* add_ge_var */
            secp256k1_gej_add_ge_var2(&tmp, &groupj[i], &group[j], NULL);
            ge_equals_gej(&group[(i + j) % order], &tmp);
            /* add_zinv_var */
    ...
    for (i = 0; i < order; i++) {
        ...
        secp256k1_gej_double_var2(&tmp, &groupj[i], NULL);
        ge_equals_gej(&group[(2 * i) % order], &tmp);
    }
    ...
    for (r_log = 1; r_log < order; r_log++) {
        for (j = 0; j < order; j++) {
            for (i = 0; i < order; i++) {
                ...
                secp256k1_ecmult2(&ctx->ecmult_ctx, &tmp, &groupj[r_log], &na, &ng);
...
int main(void) {
    ...
    for (i = 1; i < EXHAUSTIVE_TEST_ORDER; i++) {
        ...
        /* Verify against ecmult_gen */
        {
            ...
            secp256k1_ecmult_gen2(&ctx->ecmult_gen_ctx, &generatedj, &scalar_i);
