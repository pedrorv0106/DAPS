...
#define secp256k1_scalar secp256k1_scalar_t
...
static void secp256k1_ecmult_odd_multiples_table(int n, secp256k1_gej_t *prej, secp256k1_fe_t *zr, const secp256k1_gej_t *a) {
    ...
    secp256k1_gej_double_var2(&d, a, NULL);
    ...
    zr[0] = d.z;
    for (i = 1; i < n; i++) {
        secp256k1_gej_add_ge_var2(&prej[i], &prej[i-1], &d_ge, &zr[i]);
    }
...
static void secp256k1_ecmult_odd_multiples_table_storage_var(int n, secp256k1_ge_storage *pre, const secp256k1_gej *a, const secp256k1_callback *cb) {
    ...
    secp256k1_ge_t *prea = (secp256k1_ge*)checked_malloc(cb, sizeof(secp256k1_ge) * n);
...
static void secp256k1_ecmult_context_build(secp256k1_ecmult_context *ctx, const secp256k1_callback *cb) {
...
#ifdef USE_ENDOMORPHISM
    {
        ...
        g_128j = gj;
        for (i = 0; i < 128; i++) {
            secp256k1_gej_double_var2(&g_128j, &g_128j, NULL);
        }
...
*/
static int secp256k1_ecmult_wnaf2(int *wnaf, int len, const secp256k1_scalar_t *a, int w) {
...
static void secp256k1_ecmult_strauss_wnaf(const secp256k1_ecmult_context *ctx, const struct secp256k1_strauss_state *state, secp256k1_gej_t *r, int num, const secp256k1_gej_t *a, const secp256k1_scalar_t *na, const secp256k1_scalar_t *ng) {
    secp256k1_ge tmpa;
    secp256k1_fe Z;
#ifdef USE_ENDOMORPHISM
    /* Splitted G factors. */
    ...
    for (np = 0; np < num; ++np) {
        ...
        state->ps[no].bits_na     = secp256k1_ecmult_wnaf2(state->ps[no].wnaf_na,     256, &na[np],      WINDOW_A);
    ...
    if (no > 0) {
        ...
        for (np = 1; np < no; ++np) {
            secp256k1_gej_t tmp = a[state->ps[np].input_pos];
#ifdef VERIFY
...
#else
    if (ng) {
        bits_ng     = secp256k1_ecmult_wnaf2(wnaf_ng,     256, ng,      WINDOW_G);
        if (bits_ng > bits) {
            bits = bits_ng;
        }
    }
    ...
    for (i = bits - 1; i >= 0; i--) {
        int n;
        secp256k1_gej_double_var2(r, r, NULL);
#ifdef USE_ENDOMORPHISM
        for (np = 0; np < no; ++np) {
        ...
        for (np = 0; np < no; ++np) {
            if (i < state->ps[np].bits_na && (n = state->ps[np].wnaf_na[i])) {
                ECMULT_TABLE_GET_GE(&tmpa, state->pre_a + np * ECMULT_TABLE_SIZE(WINDOW_A), n, WINDOW_A);
                secp256k1_gej_add_ge_var2(r, r, &tmpa, NULL);
            }
        }
...
#ifdef USE_ENDOMORPHISM
    static const size_t point_size = (2 * sizeof(secp256k1_ge) + sizeof(secp256k1_gej_t) + sizeof(secp256k1_fe)) * ECMULT_TABLE_SIZE(WINDOW_A) + sizeof(struct secp256k1_strauss_point_state) + sizeof(secp256k1_gej_t) + sizeof(secp256k1_scalar_t);
#else
    static const size_t point_size = (sizeof(secp256k1_ge) + sizeof(secp256k1_gej_t) + sizeof(secp256k1_fe)) * ECMULT_TABLE_SIZE(WINDOW_A) + sizeof(struct secp256k1_strauss_point_state) + sizeof(secp256k1_gej_t) + sizeof(secp256k1_scalar_t);
#endif
    return n_points*point_size;
}

static int secp256k1_ecmult_strauss_batch(const secp256k1_ecmult_context *ctx, secp256k1_scratch *scratch, secp256k1_gej_t *r, const secp256k1_scalar_t *inp_g_sc, secp256k1_ecmult_multi_callback cb, void *cbdata, size_t n_points, size_t cb_offset) {
    secp256k1_gej_t* points;
    secp256k1_scalar_t* scalars;
    struct secp256k1_strauss_state state;
    size_t i;
    ...
    points = (secp256k1_gej_t*)secp256k1_scratch_alloc(scratch, n_points * sizeof(secp256k1_gej_t));
    scalars = (secp256k1_scalar_t*)secp256k1_scratch_alloc(scratch, n_points * sizeof(secp256k1_scalar_t));
    state.prej = (secp256k1_gej_t*)secp256k1_scratch_alloc(scratch, n_points * ECMULT_TABLE_SIZE(WINDOW_A) * sizeof(secp256k1_gej_t));
...
static int secp256k1_ecmult_strauss_batch_single(const secp256k1_ecmult_context *actx, secp256k1_scratch *scratch, secp256k1_gej_t *r, const secp256k1_scalar_t *inp_g_sc, secp256k1_ecmult_multi_callback cb, void *cbdata, size_t n) {
 ...
 */
static int secp256k1_wnaf_fixed(int *wnaf, const secp256k1_scalar_t *s, int w) {
    int skew = 0;
    int pos;
    int max_pos;
    int last_w;
    const secp256k1_scalar_t *work = s;
 ...
 */
static int secp256k1_ecmult_pippenger_wnaf(secp256k1_gej_t *buckets, int bucket_window, struct secp256k1_pippenger_state *state, secp256k1_gej_t *r, const secp256k1_scalar_t *sc, const secp256k1_ge *pt, size_t num) {
    ...
    for (i = n_wnaf - 1; i >= 0; i--) {
        ...
        for (np = 0; np < no; ++np) {
            ...
            if (i == 0) {
                ...
                if (skew) {
                    ...
                    secp256k1_gej_add_ge_var2(&buckets[0], &buckets[0], &tmp, NULL);
                }
            }
            if (n > 0) {
                idx = (n - 1)/2;
                secp256k1_gej_add_ge_var2(&buckets[idx], &buckets[idx], &pt[point_state.input_pos], NULL);
            } else if (n < 0) {
                idx = -(n + 1)/2;
                secp256k1_ge_neg(&tmp, &pt[point_state.input_pos]);
                secp256k1_gej_add_ge_var2(&buckets[idx], &buckets[idx], &tmp, NULL);
            }
        ...
        for(j = ECMULT_TABLE_SIZE(bucket_window+2) - 1; j > 0; j--) {
            secp256k1_gej_add_var2(&running_sum, &running_sum, &buckets[j], NULL);
            secp256k1_gej_add_var2(r, r, &running_sum, NULL);
        }

        secp256k1_gej_add_var2(&running_sum, &running_sum, &buckets[0], NULL);
        secp256k1_gej_double_var2(r, r, NULL);
        secp256k1_gej_add_var2(r, r, &running_sum, NULL);
    }
    return 1;
...
#ifdef USE_ENDOMORPHISM
SECP256K1_INLINE static void secp256k1_ecmult_endo_split(secp256k1_scalar_t *s1, secp256k1_scalar_t *s2, secp256k1_ge *p1, secp256k1_ge *p2) {
    secp256k1_scalar_t tmp = *s1;
...
#endif
    size_t entry_size = sizeof(secp256k1_ge) + sizeof(secp256k1_scalar_t) + sizeof(struct secp256k1_pippenger_point_state) + (WNAF_SIZE(bucket_window+1)+1)*sizeof(int);
    return ((1<<bucket_window) * sizeof(secp256k1_gej_t) + sizeof(struct secp256k1_pippenger_state) + entries * entry_size);
}

static int secp256k1_ecmult_pippenger_batch(const secp256k1_ecmult_context *ctx, secp256k1_scratch *scratch, secp256k1_gej_t *r, const secp256k1_scalar_t *inp_g_sc, secp256k1_ecmult_multi_callback cb, void *cbdata, size_t n_points, size_t cb_offset) {
    ...
    scalars = (secp256k1_scalar_t *) secp256k1_scratch_alloc(scratch, entries * sizeof(*scalars));
    ...
    buckets = (secp256k1_gej_t *) secp256k1_scratch_alloc(scratch, (1<<bucket_window) * sizeof(*buckets));
...
/* Wrapper for secp256k1_ecmult_multi_func interface */
static int secp256k1_ecmult_pippenger_batch_single(const secp256k1_ecmult_context *actx, secp256k1_scratch *scratch, secp256k1_gej_t *r, const secp256k1_scalar_t *inp_g_sc, secp256k1_ecmult_multi_callback cb, void *cbdata, size_t n) {
...
#endif
        space_overhead = ((1<<bucket_window) * sizeof(secp256k1_gej_t) + entry_size + sizeof(struct secp256k1_pippenger_state));
...
typedef int (*secp256k1_ecmult_multi_func)(const secp256k1_ecmult_context*, secp256k1_scratch*, secp256k1_gej_t*, const secp256k1_scalar_t*, secp256k1_ecmult_multi_callback cb, void*, size_t);
static int secp256k1_ecmult_multi_var(const secp256k1_ecmult_context *ctx, secp256k1_scratch *scratch, secp256k1_gej_t *r, const secp256k1_scalar_t *inp_g_sc, secp256k1_ecmult_multi_callback cb, void *cbdata, size_t n) {
    ...
    int (*f)(const secp256k1_ecmult_context*, secp256k1_scratch*, secp256k1_gej_t*, const secp256k1_scalar_t*, secp256k1_ecmult_multi_callback cb, void*, size_t, size_t);
    size_t max_points;
    size_t n_batches;
    ...
    if (inp_g_sc == NULL && n == 0) {
        ...
        secp256k1_scalar_t szero;
        secp256k1_scalar_set_int(&szero, 0);
        secp256k1_ecmult2(ctx, r, r, &szero, inp_g_sc);
        return 1;
    }
    ...
    for(i = 0; i < n_batches; i++) {
        ...
        secp256k1_gej_add_var2(r, r, &tmp, NULL);
        n -= nbp;
    }
    return 1;
}
