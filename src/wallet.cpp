...
bool computeStealthDestination(CKey& secret, CPubKey& pubViewKey, CPubKey& pubSpendKey, CPubKey& des) {
    ...
    //std::cout << "HS = " << temp << std::endl;
    CPubKey HSG = temp.GetPubKey();
    std::cout << "HSG = " << HSG.GetHex() << std::endl;
    ...
    std::cout << "des = " << des.GetHex() << std::endl;
    //secp256k1_stop();
    return true;
}
...
bool CWallet::SendToStealthAddress(const std::string& stealthAddr, CAmount nValue, CWalletTx& wtxNew, bool fUseIX) {
    ...
    CPubKey printed(wtxNew.txPub);
    std::cout << "transaction public key:" << printed.GetHex() << ", size = " << printed.size() << std::endl;
    ...
    std::cout << "Stealth destination:" << stealthDes.GetHex() << std::endl;
    CBitcoinAddress address(stealthDes.GetID());
    std::cout << "Bitcoin address:" << address.ToString() << std::endl;
    CScript scriptPubKey = GetScriptForDestination(address.Get());
    std::cout << "Bitcoin script:" << scriptPubKey.ToString() << std::endl;
    // Create and send the transaction
    CReserveKey reservekey(pwalletMain);
    std::cout << "reservekey:" << std::endl;
    ...
    std::cout << "after CreateTransaction:" << std::endl;
...
bool CWallet::IsTransactionForMe(const CTransaction& tx, CKey& privKey) {
    ...
    for (const CTxOut& out: tx.vout) {
        ...
        uint256 HS = Hash(aR, aR + txPubKey.size());
        unsigned char *pHS = HS.begin();
        CKey temp;
        std::cout << "Starting Set HSG" << std::endl;
        temp.Set(pHS, pHS + 32, true);
