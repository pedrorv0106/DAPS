...
enum ZerocoinSpendStatus {
    ...
    ZDAPS_SPEND_V1_SEC_LEVEL                         // Spend is V1 and security level is not set to 100
};
...
public:
    ...
    bool CreateZDAPSOutPut(libzerocoin::CoinDenomination denomination, CTxOut& outMint, CDeterministicMint& dMint);
    bool GetMint(const uint256& hashSerial, CZerocoinMint& mint);
    bool GetMintFromStakeHash(const uint256& hashStake, CZerocoinMint& mint);
    bool DatabaseMint(CDeterministicMint& dMint);
    bool SetMintUnspent(const CBigNum& bnSerial);
    bool UpdateMint(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const libzerocoin::CoinDenomination& denom);
    string GetUniqueWalletBackupName(bool fzdapsAuto) const;
    ...
    */
    ...
    CzDAPSWallet* zwalletMain;

    bool fFileBacked;
    bool fWalletUnlockAnonymizeOnly;
    std::string strWalletFile;
    bool fBackupMints;
    std::unique_ptr<CzDAPSTracker> zdapsTracker;
    ...
    void setZWallet(CzDAPSWallet* zwallet)
    {
        zwalletMain = zwallet;
        zdapsTracker = std::unique_ptr<CzDAPSTracker>(new CzDAPSTracker(strWalletFile));
    }

    CzDAPSWallet* getZWallet() { return zwalletMain; }
    ...
    /** zDAPS reset */
    boost::signals2::signal<void()> NotifyzDAPSReset;
