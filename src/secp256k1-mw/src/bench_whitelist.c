...
int main(void) {
    ...
    CHECK(secp256k1_ec_seckey_verify2(data.ctx, data.csub) == 1);
    CHECK(secp256k1_ec_pubkey_create2(data.ctx, &data.sub_pubkey, data.csub) == 1);
    ...
    for (i = 0; i < n_keys; i++) {
        ...
        CHECK(secp256k1_ec_seckey_verify2(data.ctx, data.online_seckey[i]) == 1);
        CHECK(secp256k1_ec_pubkey_create2(data.ctx, &data.online_pubkeys[i], data.online_seckey[i]) == 1);

        random_scalar_order(&soff);
        secp256k1_scalar_get_b32(data.summed_seckey[i], &soff);
        CHECK(secp256k1_ec_seckey_verify2(data.ctx, data.summed_seckey[i]) == 1);
        CHECK(secp256k1_ec_pubkey_create2(data.ctx, &data.offline_pubkeys[i], data.summed_seckey[i]) == 1);
