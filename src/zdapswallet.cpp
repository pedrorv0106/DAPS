...
CzDAPSWallet::CzDAPSWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zdaps seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZDAPSSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZDAPSSeed_deprecated()) {
                    LogPrintf("%s: Updated zDAPS seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zdaps seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zdaps wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzDAPSWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZDAPSCount(nCountLastUsed);
    else if (!walletdb.ReadZDAPSCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzDAPSWallet::Lock()
{
    seedMaster = 0;
}

void CzDAPSWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzDAPSWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZDAPS(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzDAPSWallet::LoadMintPoolFromDB()
{
...
void CzDAPSWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzDAPSWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzDAPSWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zdapsTracker->HasPubcoinHash(pMint.first)) {
...
bool CzDAPSWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZDAPS(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zdapsTracker which also adds to database
    pwalletMain->zdapsTracker->Add(dMint, true);
...
void CzDAPSWallet::SeedToZDAPS(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzDAPSWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzDAPSWallet::GenerateDeterministicZDAPS(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzDAPSWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzDAPSWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
