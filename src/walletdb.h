...
public:
    ...
    bool WriteZDAPSSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZDAPSSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZDAPSSeed_deprecated(uint256& seed);
    bool EraseZDAPSSeed();
    bool EraseZDAPSSeed_deprecated();

    bool WriteZDAPSCount(const uint32_t& nCount);
    bool ReadZDAPSCount(uint32_t& nCount);
