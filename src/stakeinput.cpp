...
CZDapsStake::CZDapsStake(const libzerocoin::CoinSpend& spend)
{
...
int CZDapsStake::GetChecksumHeightFromMint()
{
...
int CZDapsStake::GetChecksumHeightFromSpend()
{
    return GetChecksumHeight(nChecksum, denom);
}

uint32_t CZDapsStake::GetChecksum()
{
    return nChecksum;
}

// The zDAPS block index is the first appearance of the accumulator checksum that was used in the spend
// note that this also means when staking that this checksum should be from a block that is beyond 60 minutes old and
// 100 blocks deep.
CBlockIndex* CZDapsStake::GetIndexFrom()
{
    if (pindexFrom)
        return pindexFrom;
...
CAmount CZDapsStake::GetValue()
{
    return denom * COIN;
}

//Use the first accumulator checkpoint that occurs 60 minutes after the block being staked from
bool CZDapsStake::GetModifier(uint64_t& nStakeModifier)
{
...
CDataStream CZDapsStake::GetUniqueness()
{
    //The unique identifier for a zDAPS is a hash of the serial
    CDataStream ss(SER_GETHASH, 0);
    ss << hashSerial;
    return ss;
}

bool CZDapsStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
...
bool CZDapsStake::CreateTxOuts(CWallet* pwallet, vector<CTxOut>& vout, CAmount nTotal)
{
    //Create an output returning the zDAPS that was staked
    CTxOut outReward;
    libzerocoin::CoinDenomination denomStaked = libzerocoin::AmountToZerocoinDenomination(this->GetValue());
    CDeterministicMint dMint;
    if (!pwallet->CreateZDAPSOutPut(denomStaked, outReward, dMint))
        return error("%s: failed to create zDAPS output", __func__);
    vout.emplace_back(outReward);

    //Add new staked denom to our wallet
    if (!pwallet->DatabaseMint(dMint))
        return error("%s: failed to database the staked zDAPS", __func__);

    for (unsigned int i = 0; i < 3; i++) {
        CTxOut out;
        CDeterministicMint dMintReward;
        if (!pwallet->CreateZDAPSOutPut(libzerocoin::CoinDenomination::ZQ_ONE, out, dMintReward))
            return error("%s: failed to create zDAPS output", __func__);
...
bool CZDapsStake::GetTxFrom(CTransaction& tx)
{
    return false;
}

bool CZDapsStake::MarkSpent(CWallet *pwallet, const uint256& txid)
{
    CzDAPSTracker* zdapsTracker = pwallet->zdapsTracker.get();
    CMintMeta meta;
    if (!zdapsTracker->GetMetaFromStakeHash(hashSerial, meta))
        return error("%s: tracker does not have serialhash", __func__);

    zdapsTracker->SetPubcoinUsed(meta.hashPubcoin, txid);
    return true;
}
