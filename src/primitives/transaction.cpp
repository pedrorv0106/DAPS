...
CMutableTransaction::CMutableTransaction() : nVersion(CTransaction::CURRENT_VERSION), nLockTime(0), txPubKey(), hasPaymentID(0), paymentID(0) {}
CMutableTransaction::CMutableTransaction(const CTransaction& tx) : nVersion(tx.nVersion), vin(tx.vin), vout(tx.vout), nLockTime(tx.nLockTime), txPubKey(tx.txPubKey), hasPaymentID(tx.hasPaymentID), paymentID(tx.paymentID) {}
...
CTransaction::CTransaction() : hash(), nVersion(CTransaction::CURRENT_VERSION), vin(), vout(), nLockTime(0), txPubKey(), hasPaymentID(0), paymentID(0) { }

CTransaction::CTransaction(const CMutableTransaction &tx) : nVersion(tx.nVersion), vin(tx.vin), vout(tx.vout), nLockTime(tx.nLockTime), txPubKey(tx.txPubKey), hasPaymentID(tx.hasPaymentID), paymentID(tx.paymentID) {
    UpdateHash();
}
...
CTransaction& CTransaction::operator=(const CTransaction &tx) {
    ...
    *const_cast<CPubKey*>(&txPubKey) = tx.txPubKey;
